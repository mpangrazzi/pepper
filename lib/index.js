...
Pepper.prototype.logon = function(username, password, options, callback) {
  ...
  if (options.protocol) {
    ...
    var valid = ~Pepper.authProtocols.indexOf(options.protocol);

    if (!valid) {
      return callback(new TypeError('Invalid or unsupported authentication protocol'));
    }

  }

  var protocol = options.protocol || 'chap';
